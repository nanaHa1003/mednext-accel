...
def _finish_dw_kernel(partial, output, SPLITS: tl.constexpr, BLOCK: tl.constexpr):
    ...
    values = tl.load(partial + weight_index * SPLITS + offsets,
                     mask=offsets < SPLITS, other=0.0)
...
def _partial_transpose_dw_kernel(
    ...
    for start in range(0, chunk, BLOCK):
        ...
        oh = ih * 2 - padding + kh
        ow = iw * 2 - padding + kw
        valid_output = ((od >= 0) & (od < OD) &
                        (oh >= 0) & (oh < OH) &
                        (ow >= 0) & (ow < OW))
        input_index = ((n * C + channel) * D + id_) * H * W + ih * W + iw
        output_index = ((n * C + channel) * OD + od) * OH * OW + oh * OW + ow
        xv = tl.load(x + input_index, mask=valid_input, other=0.0)
        gv = tl.load(grad_output + output_index,
                     mask=valid_input & valid_output, other=0.0)
...
def _depthwise_input_grad_kernel(
        ...
        N: tl.constexpr, C: tl.constexpr,
        D: tl.constexpr, H: tl.constexpr, W: tl.constexpr,
        KERNEL_SIZE: tl.constexpr, BLOCK: tl.constexpr,
        ACCUMULATE_FP64: tl.constexpr):
    ...
    for kd in range(KERNEL_SIZE):
        for kh in range(KERNEL_SIZE):
            for kw in range(KERNEL_SIZE):
                ...
                oh = ih - kh + padding
                ow = iw - kw + padding
                valid_grad = ((od >= 0) & (od < D) &
                              (oh >= 0) & (oh < H) &
                              (ow >= 0) & (ow < W))
                grad_index = ((n * C + channel) * D + od) * H * W + oh * W + ow
                weight_index = (((channel * KERNEL_SIZE + kd) * KERNEL_SIZE + kh)
                                * KERNEL_SIZE + kw)
                gv = tl.load(grad_output + grad_index,
                             mask=valid_output & valid_grad, other=0.0)
    ...
    for kd in range(KERNEL_SIZE):
        for kh in range(KERNEL_SIZE):
            for kw in range(KERNEL_SIZE):
                ...
                divisible = ((od_numerator % 2 == 0) &
                             (oh_numerator % 2 == 0) &
                             (ow_numerator % 2 == 0))
                od = od_numerator // 2
                oh = oh_numerator // 2
                ow = ow_numerator // 2
                valid_grad = (divisible & (od >= 0) & (od < OD) &
                              (oh >= 0) & (oh < OH) &
                              (ow >= 0) & (ow < OW))
                ...
                weight_index = (((channel * KERNEL_SIZE + kd) * KERNEL_SIZE + kh)
                                * KERNEL_SIZE + kw)
                gv = tl.load(grad_output + grad_index,
                             mask=valid_input & valid_grad, other=0.0)
...
def depthwise_input_grad(grad_output, weight, block=256):
    ...
    _depthwise_input_grad_kernel[(triton.cdiv(grad_output.numel(), block),)](
        grad_output, weight, output, N=n, C=channels, D=depth, H=height, W=width,
        KERNEL_SIZE=kernel_size, BLOCK=block,
        ACCUMULATE_FP64=grad_output.dtype == torch.float64, num_warps=4)
    return output


def depthwise_stride2_input_grad(grad_output, weight, input_spatial_size,
                                 block=256):
    ...
    output = torch.empty((n, channels, depth, height, width), device=grad_output.device,
                         dtype=grad_output.dtype)
    total = output.numel()
    _depthwise_stride2_input_grad_kernel[(triton.cdiv(total, block),)](
        grad_output, weight, output, N=n, C=channels, D=depth, H=height, W=width,
        OD=out_depth, OH=out_height, OW=out_width, KERNEL_SIZE=kernel_size,
        BLOCK=block, ACCUMULATE_FP64=grad_output.dtype == torch.float64,
        num_warps=4)
    return output
...
def depthwise_weight_grad(x, grad_output, splits=64, block=512, kernel_size=3):
    ...
    _partial_dw_kernel[(weight_elements, splits)](
        x, grad_output, partial, N=n, C=channels, D=depth, H=height, W=width,
        KERNEL_SIZE=kernel_size,
        SPLITS=splits, BLOCK=block, num_warps=8)
    finish_block = triton.next_power_of_2(splits)
    _finish_dw_kernel[(weight_elements,)](
        partial, output, SPLITS=splits, BLOCK=finish_block,
        num_warps=min(8, max(1, finish_block // 32)))
    return output.reshape(channels, 1, kernel_size, kernel_size, kernel_size)


def depthwise_transpose_weight_grad(x, grad_output, splits=64, block=512,
                                    kernel_size=3):
    ...
    _partial_transpose_dw_kernel[(weight_elements, splits)](
        x, grad_output, partial, N=n, C=channels, D=depth, H=height, W=width,
        OD=grad_output.shape[2], OH=grad_output.shape[3], OW=grad_output.shape[4],
        KERNEL_SIZE=kernel_size, SPLITS=splits, BLOCK=block,
        ACCUMULATE_FP64=x.dtype == torch.float64, num_warps=8)
    finish_block = triton.next_power_of_2(splits)
    _finish_dw_kernel[(weight_elements,)](
        partial, output, SPLITS=splits, BLOCK=finish_block,
        num_warps=min(8, max(1, finish_block // 32)))
...
@triton_op('mednext::depthwise_weight_grad_odd_regular', mutates_args={})
def depthwise_weight_grad_regular(x: torch.Tensor, grad_output: torch.Tensor,
                                  kernel_size: int, channels: int,
                                  spatial_size: int, splits: int,
    ...
    wrap_triton(_partial_dw_kernel)[(weight_elements, splits)](
        x, grad_output, partial, N=1, C=channels, D=spatial_size,
        H=spatial_size, W=spatial_size, KERNEL_SIZE=kernel_size,
        SPLITS=splits, BLOCK=block, num_warps=8)
    finish_block = triton.next_power_of_2(splits)
...
@triton_op('mednext::depthwise_input_grad_odd_regular', mutates_args={})
def depthwise_input_grad_regular(grad_output: torch.Tensor, weight: torch.Tensor,
                                 kernel_size: int, channels: int,
                                 spatial_size: int, block: int) -> torch.Tensor:
    ...
    wrap_triton(_depthwise_input_grad_kernel)[(triton.cdiv(total, block),)](
        grad_output, weight, output, N=1, C=channels, D=spatial_size,
        H=spatial_size, W=spatial_size, KERNEL_SIZE=kernel_size, BLOCK=block,
        ACCUMULATE_FP64=False, num_warps=4)
    return output


@triton_op('mednext::depthwise_input_grad_stride2_regular', mutates_args={})
def depthwise_input_grad_stride2_regular(
        grad_output: torch.Tensor, weight: torch.Tensor, channels: int,
        spatial_size: int, block: int) -> torch.Tensor:
    """Compiled dX for one statically configured stride-two depthwise shape."""
    output = torch.empty((1, channels, spatial_size, spatial_size, spatial_size),
    ...
    wrap_triton(_depthwise_stride2_input_grad_kernel)[(triton.cdiv(total, block),)](
        grad_output, weight, output, N=1, C=channels, D=spatial_size,
        H=spatial_size, W=spatial_size, OD=output_size, OH=output_size,
        OW=output_size, KERNEL_SIZE=weight.shape[2], BLOCK=block,
        ACCUMULATE_FP64=False, num_warps=4)
    return output


@custom_op('mednext::depthwise_conv3d_odd_regular', mutates_args=())
def depthwise_conv3d_regular(x: torch.Tensor, weight: torch.Tensor,
                             bias: torch.Tensor | None, kernel_size: int,
                             channels: int, spatial_size: int, dw_splits: int,
                             dw_block: int, dx_block: int) -> torch.Tensor:
    return torch.nn.functional.conv3d(
        x, weight, bias, padding=kernel_size // 2, groups=channels)
...
def _regular_backward(ctx, grad_output):
    ...
    grad_input = depthwise_input_grad_regular(
        grad_output, weight, ctx.kernel_size, ctx.channels, ctx.spatial_size,
        ctx.dx_block)
    grad_weight = depthwise_weight_grad_regular(
        x, grad_output, ctx.kernel_size, ctx.channels, ctx.spatial_size,
        ctx.dw_splits, ctx.dw_block)
    grad_bias = None
    if ctx.has_bias:
        padding = ctx.kernel_size // 2
        _, _, grad_bias = torch.ops.aten.convolution_backward(
            grad_output, x, weight, [ctx.channels],
            [1, 1, 1], [padding, padding, padding], [1, 1, 1],
            False, [0, 0, 0], ctx.channels, [False, False, True])
    return grad_input, grad_weight, grad_bias, None, None, None, None, None, None


depthwise_conv3d_regular.register_autograd(
    _regular_backward, setup_context=_setup_regular_context)


@custom_op('mednext::depthwise_conv3d_stride2_regular', mutates_args=())
def depthwise_conv3d_stride2_regular(
        x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor | None,
        kernel_size: int, channels: int, spatial_size: int, dx_block: int) -> torch.Tensor:
    return torch.nn.functional.conv3d(
...
def _(x, weight, bias, kernel_size, channels, spatial_size, dx_block):
    output_size = (spatial_size + 1) // 2
    return torch.empty((x.shape[0], channels, output_size, output_size, output_size),
...
def _stride2_backward(ctx, grad_output):
    ...
    _, grad_weight, grad_bias = torch.ops.aten.convolution_backward(
        grad_output, x, weight, [ctx.channels] if ctx.has_bias else None,
        [2, 2, 2], [padding, padding, padding], [1, 1, 1], False, [0, 0, 0],
        ctx.channels, [False, True, ctx.has_bias])
...
@triton_op('mednext::depthwise_transpose_weight_grad_odd_regular', mutates_args={})
def depthwise_transpose_weight_grad_regular(
        x: torch.Tensor, grad_output: torch.Tensor, kernel_size: int,
        channels: int, spatial_size: int, splits: int, block: int) -> torch.Tensor:
    ...
    wrap_triton(_partial_transpose_dw_kernel)[(weight_elements, splits)](
        x, grad_output, partial, N=1, C=channels, D=spatial_size,
        H=spatial_size, W=spatial_size, OD=output_size, OH=output_size,
        OW=output_size, KERNEL_SIZE=kernel_size, SPLITS=splits, BLOCK=block,
        ACCUMULATE_FP64=False, num_warps=8)
    finish_block = triton.next_power_of_2(splits)
    wrap_triton(_finish_dw_kernel)[(weight_elements,)](
        partial, output, SPLITS=splits, BLOCK=finish_block,
        num_warps=min(8, max(1, finish_block // 32)))
...
@custom_op('mednext::depthwise_conv_transpose3d_odd_regular', mutates_args=())
def depthwise_conv_transpose3d_regular(
        x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor | None,
        kernel_size: int, channels: int, spatial_size: int, dw_splits: int,
        dw_block: int) -> torch.Tensor:
...
def _transpose_regular_backward(ctx, grad_output):
    ...
    grad_input, _, grad_bias = torch.ops.aten.convolution_backward(
        grad_output, x, weight, [ctx.channels] if ctx.has_bias else None,
        [2, 2, 2], [padding, padding, padding], [1, 1, 1], True, [0, 0, 0],
        ctx.channels, [True, False, ctx.has_bias])
    grad_weight = depthwise_transpose_weight_grad_regular(
        x, grad_output, ctx.kernel_size, ctx.channels, ctx.spatial_size,
        ctx.dw_splits, ctx.dw_block)
    return grad_input, grad_weight, grad_bias if ctx.has_bias else None, None, None, None, None, None
...
class SplitDepthwiseConv3d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if (config is not None and
                (self.selected_shapes is None or (x.shape[1], x.shape[2]) in self.selected_shapes)
                and x.shape == (1, x.shape[1], x.shape[2],
                                                x.shape[2], x.shape[2])
                and x.is_contiguous()):
            ...
            return depthwise_conv3d_regular(
                x, weight, bias, self.kernel_size, x.shape[1], x.shape[2],
                dw_splits, dw_block, dx_block)
        return torch.nn.functional.conv3d(
...
def replace_highres_depthwise_convs(model, include_stride2_input_grad=False,
                                    policy=None):
    ...
    for name, child in list(model.named_children()):
        ...
        cubic = (len(set(child.kernel_size)) == 1
                 if (is_conv3d or is_transpose3d) else False)
        kernel_size = child.kernel_size[0] if cubic else 0
        regular = (is_conv3d and child.in_channels in _REGULAR_CHANNELS
                and child.out_channels == child.in_channels
                and child.groups == child.in_channels
                and cubic and kernel_size % 2 == 1
                and child.stride == (1, 1, 1)
                and child.padding == (kernel_size // 2,) * 3
                and child.dilation == (1, 1, 1)
                and child.padding_mode == 'zeros')
        transpose = (is_transpose3d and child.in_channels in _TRANSPOSE_CHANNELS
                     and child.out_channels == child.in_channels
                     and child.groups == child.in_channels
                     and cubic and kernel_size % 2 == 1
                     ...
                     and child.dilation == (1, 1, 1))
        downsample = (is_conv3d and child.in_channels in _DOWNSAMPLE_CHANNELS
                      and child.out_channels == child.in_channels
                      and child.groups == child.in_channels
                      and cubic and kernel_size % 2 == 1
                      and child.stride == (2, 2, 2)
        ...
        if regular:
            ...
            count += replace_highres_depthwise_convs(
                child, include_stride2_input_grad, policy)
    return count
