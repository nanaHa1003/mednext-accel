...
class EvalModeGELU(nn.Module):
    """Use exact GELU for training and optionally tanh GELU for evaluation."""
    def __init__(self, approximate_eval: bool = False):
        super().__init__()
        self.approximate_eval = approximate_eval

    def forward(self, x: Tensor) -> Tensor:
        approximate = 'tanh' if self.approximate_eval and not self.training else 'none'
...
class MedNeXt(nn.Module):
    def __init__(
        ...
        for module in self.modules():
            if isinstance(module, EvalModeGELU):
                module.approximate_eval = approximate_gelu_eval
